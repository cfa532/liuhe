...
import { useAlertStore, useLeitherStore, useCaseListStore, useCaseStore } from '@/stores'
    ...
    actions: {
        async login(username: string, password: string) {
            try {
                ...
                this.user.mid = await lapi.client.RunMApp("update_user", {aid: lapi.appId, ver: "last",
                ...
                localStorage.setItem("userMid", this.user.mid)
        ...
        async logout() {
            try {
                ...
                lapi.client.RunMApp("logout", {
                    aid: lapi.appId, ver: "last", userid: this.user.mid}
                )
                useCaseStore().$reset()
