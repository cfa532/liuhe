...
async function submitQuery(e: MouseEvent) {
    ...
    console.log(userRole.value, userTask.value, subTask.value, submitButton.value.innerText)
    if (submitButton.value.innerText == "Confirm") {
        // user has edited the result generated by AI, now same it to database and move to next task
...
<div class="container d-grid row-gap-3">
    <div class="row">
        ...
        <div class="card fs-6">
            ...
            <ul class="list-group list-group-flush">
                <li class="list-group-item text-secondary">{{ caseStore.case.value.brief }}</li>
                <li class="list-group-item text-secondary">原告：{{ caseStore.case.value.plaintiff }}</li>
                <li class="list-group-item text-secondary">被告：{{ caseStore.case.value.defendant }}</li>
                <li v-if="caseStore.case.value.judge" class="list-group-item text-secondary">主审：{{ caseStore.case.value.judge }}</li>
            </ul>
        </div>
        </div>
    </div>
    ...
    <div v-show="AiContent" class="row mt-2">
        ...
        <p></p>
        <div class="col">
            <button @click.prevent="confirmAiResult" style="position: relative; float: right;">保存</button>
        </div>
    </div>
</div>
</form>
