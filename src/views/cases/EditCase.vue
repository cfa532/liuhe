...
const showAIContent=ref(AiContent.value)
...
async function submitQuery(e: MouseEvent) {
    ...
    if (submitButton.value.innerText == "Confirm") {
        ...
        await caseStore.addTemplateItem(userRole.value+":"+userTask.value+":"+subTask.value, AiContent.value)
    } else {
            ...
            case "attorney":
                switch(userTask.value) {
                    case "t1":      // 民事起诉
                        ...
                        submitButton.value.innerText = "Confirm"
                        break;
                    case "t2":      // 民事答辩
                        break;
                    default:        // 刑事诉讼
...
<div class="container d-grid row-gap-3">
    ...
    <div v-show="showPrompt" class="row mt-2">
        ...
        <p></p>
        <div class="col">
            <button ref="submitButton" @click.prevent="submitQuery" style="position: relative; float: right;">提交</button>
        </div>
    </div>
    <div v-show="showAIContent" class="row mt-2">
        <textarea rows="8" class="col" v-model="AiContent"></textarea>
        <p></p>
        <div class="col">
            <button ref="submitButton" @click.prevent="acceptResult" style="position: relative; float: right;">保存</button>
        </div>
    </div>
</div>
</form>
